...
class GraphACOTest(unittest.TestCase):
    ...
    def test_run(self):
        graph = Graph('../input/50_airports_input.csv')
        solver = GraphACO(graph.G, ants=50, alpha=0.25, rho=0.25, phe_reinit=100, evaporation=False)
        path, cost = solver.search(2500)
